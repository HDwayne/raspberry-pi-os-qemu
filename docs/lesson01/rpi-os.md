...
Note: All the future experiments will have the same code structure.

### Makefile walkthrough

If you are not familiar with Makefiles, read [this article](http://opensourceforu.com/2012/06/gnu-make-in-detail-for-beginners/).
...
* `-nostdlib` Don't use the C standard library. Most of the calls in the C standard library eventually interact with the operating system via system calls. We are writing a bare-metal program, and we don't have any underlying operating system, so the C standard library is not going to work for us anyway.
* `-fno-stack-protector` Disable the generation of extra code to help preventing buffer overflows. This makes sure that the generated code remains simple, and as I write these lines, this extra code calls the C standard library so we cannot use it anyway.
...
* `-mgeneral-regs-only`. Use only general-purpose registers. ARM processors also have [NEON](https://developer.arm.com/technologies/neon) registers. We do not want the compiler to use them because they add additional complexity (since, for example, we will need to store the registers during a context switch).
* `-g` Include debugging info in the resultant ELF binary. For debugging sanity (to know the provenance of the instructions your debugger executes).
* `-O0` Turn off any compiler optimization. For ease of debugging.
* `-DUSE_QEMU` Define the `USE_QEMU` macro. As QEMU and real Raspi3 hardware do not have the exact same booting process, this enables our code to work on both platforms.
...
We use the `OBJ_FILES` array to build the `kernel8.elf` file. We use the linker script `src/linker-qemu.ld` to define the basic layout of the resulting executable image (we will discuss the linker script in the next section).
...
Despite so, the community figured out most of the SoC details over years because of Rpi3's popularity. It's not our goal to dive in the SoC. Rather, our philosophy is to deal BCM2837-specific details as few as possible -- just enough to get our kernel working. We will spend more efforts on explaining generic hardware such as ARM64 cores, generic timers, irq controllers, etc.
...
Here, we use the two functions `put32` and `get32`. Those functions are very simple -- read and write some data to and from a 32-bit register. You can take a look at how they are implemented in [utils.S](../../src/lesson01/src/utils.S). `uart_init` is one of the most complex and important functions in this lesson, and we will continue to examine it in the next three sections.
